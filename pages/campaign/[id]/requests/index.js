...
const RequestRow = ({
  ...
  const onReject = async () => {
    ...
    try {
      const campaign = Campaign(campaignId);
      const accounts = await web3.eth.getAccounts();
      await campaign.methods.rejectRequest(id).send({
        from: accounts[0],
      });
      router.reload();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {/* it is for rejection */}
      <Td>
        {request.rejectedCount}/{approversCount}
      </Td>
      <Td>
        <HStack spacing={2}>
          <Tooltip
          ...
          {request.complete ? (
            ...
            >
              <CheckCircleIcon
                color={useColorModeValue("#F27920.600", "#F27920.300")}
              />
            </Tooltip>
      ...
      <main>
        <Container px={{ base: "4", md: "12" }} maxW={"7xl"} align={"left"}>
          <Flex flexDirection={{ base: "column", md: "row" }} py={4}>
            <Box py="4">
              <Text fontSize={"lg"} color={"rgba(210,80,40,0.9)"}>
                <ArrowBackIcon mr={2} />
